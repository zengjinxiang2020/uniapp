		...
		methods: {
				...
				uni.getSetting({
					success(res) {
						...
						if (res.authSetting['scope.userLocation']) {
							console.log("getSetting authorize")
							that.getLocation()
							that.findShop()
						} else {
							console.log("getSetting unauthorize")
							that.getAuthorize()
						}
					}
				})
			...
			getLocation() {
				...
				uni.getLocation({
					...
					fail: function(res) {
						console.error("getLocation失败" + res.status);
					},
				});
			},
			findShop() { //拿到商家的地理位置，用到了腾讯地图的api
				...
				qqmapsdk.calculateDistance({
					...
					success: function(res) { //成功后的回调
						...
						console.log("res.elements.length" + res.elements.length);
